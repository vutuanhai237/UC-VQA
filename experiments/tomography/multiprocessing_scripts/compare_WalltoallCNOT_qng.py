...
def run_walltoall(num_layers, num_qubits):
    ...
    for i in range(0, 400):
        ...
        G = qsee.gradient.qng(qc.copy(), thetas, qsee.ansatz.create_WalltoallCNOT_layerd_state, num_layers)
        grad_loss = qsee.measure.grad_loss(
            qc, 
            qsee.ansatz.create_WalltoallCNOT_layerd_state,
    ...
    for thetas in thetass:
        qc = qiskit.QuantumCircuit(num_qubits, num_qubits)
        qc = qsee.ansatz.create_WalltoallCNOT_layerd_state(qc, thetas, num_layers = num_layers).inverse()
