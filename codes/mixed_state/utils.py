# This code is forked from https://github.com/yiminghwang/qWGAN
...
def _unflatten(flat, prototype):
    ...
    if isinstance(prototype, np.ndarray):
        ...
        raise TypeError('Unsupported type in the model: {}'.format(
            type(prototype)))
...
def unflatten(flat, prototype):
    ...
    if len(tail) != 0:
        raise ValueError(
            'Flattened iterable has more elements than the model.')
    return result
...
def getreal_denmat(system_size, num_to_mix, prob_real, input_state):
    real_state_denmat = np.asmatrix(
        np.zeros((2**system_size, 2**system_size), dtype=complex))
    # linear combination of pure state
    for i in range(num_to_mix):
        print(input_state[i])
        print('...')
        real_state_denmat += prob_real[i] * (np.matmul(input_state[i],
                                                       input_state[i].getH()))
    return real_state_denmat

def getreal_vector(system_size, num_to_mix, prob_real, input_state):
    real_state_denmat = np.zeros((2**system_size, 1), dtype=complex)
    # linear combination of pure state
    for i in range(num_to_mix):
        real_state_denmat += prob_real[i] * input_state[i]
    return real_state_denmat
