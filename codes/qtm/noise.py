...
def generate_noise_model(num_qubit: int, error_prob: float):
    ...
    for qi in range(num_qubit):
        read_err = qiskit.providers.aer.noise.errors.readout_error.ReadoutError(
...
def generate_measurement_filter(num_qubits, noise_model):
    ...
    meas_cals, state_labels = qiskit.ignis.mitigation.measurement.complete_meas_cal(qubit_list=range(
    ...
    meas_filter = qiskit.ignis.mitigation.measurement.CompleteMeasFitter(cal_results, state_labels).filter
    return meas_filter