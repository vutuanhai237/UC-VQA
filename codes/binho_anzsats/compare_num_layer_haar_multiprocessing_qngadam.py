...
def run_haar(num_layers, num_qubits):
    ...
    np.savetxt("../../experiments/binho_ansatz_15layer_qngadam/" +
               str(num_layers) + "/loss_values_haar.csv", loss_values_haar, delimiter=",")
    np.savetxt("../../experiments/binho_ansatz_15layer_qngadam/" +
               str(num_layers) + "/thetass_haar.csv", thetass_haar, delimiter=",")
    np.savetxt("../../experiments/binho_ansatz_15layer_qngadam/" +
               str(num_layers) + "/traces_haar.csv", traces_haar, delimiter=",")
    np.savetxt("../../experiments/binho_ansatz_15layer_qngadam/" +
               str(num_layers) + "/fidelities_haar.csv", fidelities_haar, delimiter=",")
...
if __name__ == "__main__":
    ...
    for i in num_layers:
        t_haar.append(multiprocessing.Process(
            target=run_haar, args=(i, num_qubits)))
