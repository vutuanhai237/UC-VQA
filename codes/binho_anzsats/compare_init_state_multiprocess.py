...
def run_ghz(num_qubits, iter):
    ...
    for i in range(0, iter):
        ...
        qc_copy = qtm.nqubit.create_GHZchecker_binho(
    ...
    np.savetxt("../../experiments/binho_ansatz_200iter/" + str(num_qubits) +
               "/loss_values_ghz.csv", loss_values_ghz, delimiter=",")
    np.savetxt("../../experiments/binho_ansatz_200iter/" +
               str(num_qubits) + "/thetass_ghz.csv", thetass_ghz, delimiter=",")
    np.savetxt("../../experiments/binho_ansatz_200iter/" +
               str(num_qubits) + "/traces_ghz.csv", traces_ghz, delimiter=",")
    np.savetxt("../../experiments/binho_ansatz_200iter/" + str(num_qubits) +
               "/fidelities_ghz.csv", fidelities_ghz, delimiter=",")
...
def run_w(num_qubits, iter):
    ...
    print('Writting ...')
    np.savetxt("../../experiments/binho_ansatz_200iter/" +
               str(num_qubits) + "/loss_values_w.csv", loss_values_w, delimiter=",")
    np.savetxt("../../experiments/binho_ansatz_200iter/" +
               str(num_qubits) + "/thetass_w.csv", thetass_w, delimiter=",")
    np.savetxt("../../experiments/binho_ansatz_200iter/" +
               str(num_qubits) + "/traces_w.csv", traces_w, delimiter=",")
    np.savetxt("../../experiments/binho_ansatz_200iter/" +
               str(num_qubits) + "/fidelities_w.csv", fidelities_w, delimiter=",")
...
def run_haar(num_qubits, iter):
    ...
    for i in range(0, iter):
        ...
        thetas = np.real(thetas - qtm.constant.learning_rate *
                         (np.linalg.inv(G) @ grad_loss))
        qc_copy = qtm.nqubit.create_haarchecker_binho(
